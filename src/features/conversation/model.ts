...
const QUEUE_EDIT_PLACEHOLDER = 'Queued prompt text'
const DEFAULT_SEND_MODE: ConversationSendMode = 'queue'
const ALTERNATE_SEND_MODE: Readonly<Record<ConversationSendMode, ConversationSendMode>> = Object.freeze({
  queue: 'steer',
  steer: 'queue',
})
...
function resolveSendMode(
  ...
  gesture: ConversationSubmitGesture,
): ConversationSendMode {
  if (!running) return DEFAULT_SEND_MODE
  return gesture === 'primary' ? behavior : ALTERNATE_SEND_MODE[behavior]
}
...
class ConversationControllerService implements ConversationController {
  ...
  private busyEnter: ConversationSendMode = DEFAULT_SEND_MODE
  private busyEnterBusy = false
  ...
  private queueBusy: MessageId | undefined
  ...
  constructor(options: ConversationControllerOptions) {
    ...
    if (options.preferences !== undefined) {
      this.resources.push(options.preferences.subscribe(() => { void this.refreshPreferences() }))
  ...
  beginQueueEdit(itemId: MessageId): void {
    ...
    const item = snapshot?.queue.find(candidate => candidate.id === itemId && candidate.placement === 'queued')
    if (snapshot === undefined || snapshot.removed || snapshot.subagent !== null || item?.text === null || item?.text === undefined) return
    this.triggers?.dismiss()
    this.mediaState.cancelInput()
    this.queueEdit = { id: itemId, value: sanitizeConversationText(item.text) }
  ...
  getSnapshot(): ConversationSnapshotView {
    ...
    return Object.freeze({
      ...
      busyEnterAvailable: this.busyEnterRevision !== undefined,
      busyEnterBusy: this.busyEnterBusy,
      ...
      input: this.queueEdit === undefined
        ...
        : {
            kind: 'queue-edit' as const,
            placeholder: QUEUE_EDIT_PLACEHOLDER,
            title: QUEUE_EDIT_TITLE,
            value: this.queueEdit.value,
          },
      ...
      phase,
      primarySendMode: resolveSendMode(snapshot?.running ?? false, this.busyEnter, 'primary'),
      queue,
  ...
  async removeQueue(itemId: MessageId): Promise<boolean> {
    return await this.mutateQueue(itemId, { kind: 'remove' }) === 'applied'
  }
  ...
  sendAlternateDraft(): Promise<boolean> {
    const running = this.binding?.getSnapshot().running ?? false
    return this.sendDraft(resolveSendMode(running, this.busyEnter, 'alternate'))
  }
  ...
  async sendDraft(mode?: ConversationSendMode): Promise<boolean> {
    ...
    const resolvedMode = mode ?? resolveSendMode(binding.getSnapshot().running, this.busyEnter, 'primary')
    ...
    const sent = await this.runAction(async () => {
      ...
      await binding.prompt(content, resolvedMode)
    })
    this.sending = false
  ...
  setInput(value: string): void {
    if (this.queueEdit !== undefined) {
      this.queueEdit.value = sanitizeConversationText(value)
      return
    }
    this.mediaState.setInput(value)
  }

  async steerQueue(itemId: MessageId): Promise<boolean> {
    return await this.mutateQueue(itemId, { kind: 'steer' }) !== 'failed'
  }

  async steerQueueAll(): Promise<boolean> {
    const queued = this.binding?.getSnapshot().queue.filter(item => item.placement === 'queued') ?? []
    if (queued.length === 0) return false
    for (const item of queued) {
      const outcome = await this.mutateQueue(item.id, { kind: 'steer' })
  ...
  async submitInput(): Promise<boolean> {
    ...
    if (edit === undefined) return this.mediaState.submitInput()
    if (sanitizeConversationText(edit.value).trim() === '') return false
    const updated = await this.mutateQueue(edit.id, {
      kind: 'edit',
      content: [{ type: 'text', text: sanitizeConversationText(edit.value) }],
    }) === 'applied'
    if (updated && this.queueEdit === edit) {
      this.queueEdit = undefined
      this.schedulePublish()
    }
    return updated
  ...
  async toggleBusyEnter(): Promise<boolean> {
    ...
    if (preferences === undefined || revision === undefined || this.busyEnterBusy) return false
    const behavior = ALTERNATE_SEND_MODE[this.busyEnter]
    this.busyEnterBusy = true
    this.schedulePublish()
    try {
      const result = await preferences.write(behavior, revision)
      if (result.ok) {
        this.busyEnter = result.value.behavior
        this.busyEnterRevision = result.value.revision
        this.error = undefined
        return true
      }
      this.error = sanitizeText(`${result.error.code}: ${result.error.message}`)
      return false
    } catch (error) {
      this.error = sanitizeText(error instanceof Error ? error.message : String(error))
      return false
    } finally {
      this.busyEnterBusy = false
      this.schedulePublish()
    }
  }
  ...
  private syncQueueState(): void {
    ...
    const snapshot = this.binding?.getSnapshot()
    const remainsEditable = snapshot !== undefined
      && !snapshot.removed
      && snapshot.subagent === null
      && snapshot.queue.some(item => item.id === edit.id && item.placement === 'queued' && item.text !== null)
    if (!remainsEditable) this.queueEdit = undefined
  }
  ...
  private async mutateQueue(itemId: MessageId, action: QueueAction): Promise<QueueMutationOutcome> {
    ...
    this.schedulePublish()
    try {
      const result = await binding.updateQueue(itemId, action)
      if (result.ok) {
        this.error = undefined
        return 'applied'
      }
      if (action.kind === 'steer' && STEER_CONVERGENCE_CODES.has(result.error.code)) {
        this.error = undefined
        return 'converged'
      }
      this.error = sanitizeText(`${result.error.code}: ${result.error.message}`)
      ...
      if (this.queueBusy === itemId) this.queueBusy = undefined
      this.schedulePublish()
    }
  }
  ...
  private async refreshPreferences(): Promise<void> {
    ...
    try {
      ...
      if (this.disposed || generation !== this.preferenceGeneration) return
      this.error = sanitizeText(error instanceof Error ? error.message : String(error))
      this.schedulePublish()
      return
    }
