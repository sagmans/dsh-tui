/**
 * The frames every golden suite renders: the widths they are drawn at, the
 * terminal stub they are measured on, and the fixtures built for each case.
 */
...
export function terminalTextFrame(frameTheme = theme): TranscriptView {
  const model = new TranscriptModel({
