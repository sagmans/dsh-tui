...
describe('transcript band accents', () => {
  it.each([
    ...
    expect(rows[1]).toBe(accent + '◜' + RESET + rule + '──────────' + RESET + accent + '◝' + RESET)
    expect(rows.at(-2)).toBe(accent + '◟' + RESET + rule + '──────────' + RESET + accent + '◞' + RESET)
  ...
  it.each(['deepseek-blue', 'violet-orbit'])('keeps quiet rules and speaker accents under %s', themeName => {
    for (const role of ['user', 'assistant'] as const) {
      ...
      expect(rows[1]).toBe(MARK[role] + '◜' + RESET + RULE[role] + '──────────' + RESET + MARK[role] + '◝' + RESET)
      expect(rows.at(-2)).toBe(MARK[role] + '◟' + RESET + RULE[role] + '──────────' + RESET + MARK[role] + '◞' + RESET)
  ...
  it('keeps submitted-prompt rules independent of the boxed editor colour', () => {
    ...
    expect(render('user', 'truecolor', tokens)[1]).toBe(MARK.user + '◜' + RESET + RULE.user + '──────────' + RESET + MARK.user + '◝' + RESET)
  })
  ...
  it.each(['user', 'assistant'] as const)('lets the %s mark override leave the rule and body alone', role => {
    ...
    expect(rows[1]).toBe(RED + '◜' + RESET + RULE[role] + '──────────' + RESET + RED + '◝' + RESET)
    expect(rows[2]).toBe(role === 'user' ? MARK.user + TEXT + RESET : TEXT)
  })
  ...
  it('reserves the mark columns when the mark element is hidden', () => {
    ...
    expect(rows[1]).toBe(' ' + RULE.assistant + '──────────' + RESET + ' ')
