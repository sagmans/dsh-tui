...
| a call that runs for more than a second, watched while it runs | its row ends in `▸ ~1s` and the seconds keep counting, with the command still on the row; the mark gives way to the measured facts the moment the result lands |
| a PTC turn with a slow dispatched call | that call's indented row carries `▸` while the program waits for it, and loses it when the dispatch log lands |
| `dsh-tui: { tokens: { tool.running.title: { hidden: true } } }` in `$DSH_HOME/settings.yaml`, then a slow call | the row draws no mark and no duration, and the call still settles into its result |
