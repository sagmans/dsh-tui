...
Tool cards are folded by default: a card draws one header row — the tool, its argument clipped to the configured budget, and the facts the result measured — so a long read, diff, or search cannot bury the conversation. A call that has not answered yet draws `▸` in the `tool.running.glyph` style in front of the tool's name, and ends that row with the whole seconds it has been waiting, in `tool.running.elapsed`, once it has waited one, so a reader can tell the call they are watching from the one below it that already finished; both give way to the measured facts the moment the result lands. A shell card's row also carries the exit status and the count of output rows waiting behind the fold, because its output is the answer the reader asked for and a fold that left no trace of it would read as a call that produced nothing. Clicking a card opens or folds that one message; `Ctrl+O` opens or folds every card at once, and `tools:` in the [settings](#settings) decides how each tool starts and whether a fold hides its rows or keeps a `tail` of them.

A PTC card is the one card with children: every call the `run_code` program dispatched hangs off the card that made it, and each draws under the header as the tool's own name and argument, on one row cut at the screen edge whether the card itself is open or folded — a program's work must stay legible without opening its card. Every one of those rows opens with what became of its own call, in front of the name so the names stay in one column: `▸` in `tool.subcall.running` while the program is waiting on it, `✓` in `tool.subcall.done` once its log lands, and `✗` in `tool.subcall.failed` when that call reported a failure — the state is a shape as well as a colour, so a program's work reads row by row without opening the card. A row that has settled also keeps the one line the tool itself drew about its own outcome, in `tool.terminal.status` where that tool declares one: a shell call reads `✓ bash pnpm test · exit 0`, so the command that quietly exited non-zero is as visible as the one that failed outright. The program's own row is the one card that shows no mark: its timer is what says it is still running, and it is also what the card keeps when the program answers — the total it ran for, drawn in `tool.elapsed.done`, dimmed and italic, because the work it measured is over. Clicking one of those rows opens that call's argument in full and leaves its neighbours and the card as they were; a shell call also brings back the rows it printed, because the program's return value is all the card itself keeps. `Ctrl+Y` hides or shows them all, and `subcalls: collapsed` starts every session with them hidden; see [Settings](#settings).

A card's header names the tool, then the argument the call was made with — a path or a command — in the `tool.args` colour, then the facts the result measured: a read reports its line range, line count, and token size; a file change that carried no prior content to compare against reports its lines and tokens; one that did reports added, changed, and removed lines as `+n ~n -n` in green, yellow, and red. Each stat is its own token, so any of them can be recoloured or hidden independently, and so are the elements a call in flight is drawn with — hiding `tool.running.glyph` leaves the seconds counting, hiding `tool.running.elapsed` leaves the mark, hiding `tool.elapsed.done` takes the total off a program's settled row, and a dispatched row's state can go the same way per state with `tool.subcall.running`, `tool.subcall.done`, and `tool.subcall.failed`.
...
| a call that runs for more than a second, watched while it runs | its row reads `▸ <command>` with `~1s` counting up at the end, the command still on the row; both give way to the measured facts the moment the result lands |
| a PTC turn with several dispatched calls, one of them slow, one of them exiting non-zero | each indented row opens with its own state — `▸` while the program waits on it, `✓` once it lands, `✗` if it failed — and a settled row keeps its own outcome, such as `exit 1` |
| a PTC turn whose program ran for more than a second | the card's row carries no mark while it runs, only the counting seconds; once the program answers, the total stays on the row dimmed and italic |
| `dsh-tui: { tokens: { tool.running.glyph: { hidden: true } } }` in `$DSH_HOME/settings.yaml`, then a slow call | the row draws no mark and keeps counting the seconds, and the call still settles into its result |
| `dsh-tui: { tokens: { tool.running.elapsed: { hidden: true } } }`, then a slow call | the row keeps its `▸` and reports no duration |
