...
Transcript messages have top and bottom rules, a braille dot at either end of each, and one blank row inside each rule. There are no side borders or horizontal padding, so copied message rows contain only the text. By default the dots carry the colour — mint for your prompts, warm gold for replies — and each rule between them is that same hue dimmed almost to the background, so a message reads as its words with four small dots around them rather than as a fence, and the rule still says whose turn it closes. A dot sits low in its cell on the rule that opens and high on the rule that closes, so the pair leans over the message and two blocks in a row stay two. Dots rather than the arcs a box closes with, because any drawn mark is read as a shape at the end of the rule and a shape promises the sides and the padding a band deliberately has none of; a dot is bare position, and the rule runs unbroken to its own end. The editor and queued prompts keep their full boxes, so the shape still says what is still being typed into and what is already said.
...
- `transcript.user.mark` and `transcript.assistant.mark` style the four dots independently. A hidden mark leaves a blank column, so the rule keeps its position.
