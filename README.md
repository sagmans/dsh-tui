...
Transcript messages have top and bottom rules, a quadrant arc at either end of each, and a blank row outside each rule. There are no side borders, no horizontal padding, and no blank row between a message and its own rules, so copied message rows contain only the text and the shape asks for nothing the text does not have. By default the arcs carry the colour — mint for your prompts, warm gold for replies — and each rule between them is that same hue dimmed almost to the background, so a message reads as its words with four small marks around them rather than as a fence, and the rule still says whose turn it closes. The arcs sit outside their rules — lifting over the rule that opens a block and dropping under the one that closes it — and the blank rows sit outside those, so the space around a message separates it from the next block instead of padding it. Arcs detached from the rule rather than the arcs a box closes with, because an arc joined to the rule is read as a corner, and a corner promises the sides and the padding a band deliberately has none of. The editor and queued prompts keep their full boxes, so the shape still says what is still being typed into and what is already said.
...
- `transcript.user.mark` and `transcript.assistant.mark` style the four arcs independently. A hidden mark leaves a blank column, so the rule keeps its position.
