...
Transcript messages have top and bottom rules, a quotation ornament at either end of each, and one blank row inside each rule. There are no side borders or horizontal padding, so copied message rows contain only the text. By default the ornaments carry the colour — mint for your prompts, warm gold for replies — and each rule between them is that same hue dimmed almost to the background, so a message reads as its words with four small marks around them rather than as a fence, and the rule still says whose turn it closes. The rule that opens a band faces its ornaments inward and the rule that closes it wears them turned around, so two blocks in a row stay two. Ornaments rather than the arcs a box closes with, because an arc is read as a corner and a corner promises the sides and the padding a band deliberately has none of; a quotation is the other reading a rule's end can carry, and a message in a transcript is something said. The editor and queued prompts keep their full boxes, so the shape still says what is still being typed into and what is already said.
...
- `transcript.user.mark` and `transcript.assistant.mark` style the four ornaments independently. A hidden mark leaves a blank column, so the rule keeps its position.
